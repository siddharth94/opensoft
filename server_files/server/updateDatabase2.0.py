...
def keywords(text_article) :
    common_words = open("common_words.txt").read().split(' ')
    word_dict = {}
    word_list = text_article.lower().split()
    for word in word_list :
        if word not in common_words and word.isalnum():
            if word not in word_dict:
                word_dict[word] = 1
            else :
                word_dict[word] += 1
    top_words = sorted(word_dict.items(),key=lambda(k,v):(v,k),reverse=True)[0:5]
    ans = []
    for w in top_words :
        ans.append(w[0])    
